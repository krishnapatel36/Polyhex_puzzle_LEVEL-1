...
def solutions_svg(solutions, filename, date_number, date_position, columns=1, size=20, padding=20,
                  colour=lambda _: "white", stroke_colour="black",
                  stroke_width=30, empty=' '):
    solutions = list(solutions)
    height, width = solutions[0].size
    rows = (len(solutions) + columns - 1) // columns
...
date_position = date_map[number]
solutions_svg([all_solutions[0]], filename='first_solution.svg', date_number=number, date_position=date_position, columns=7, colour=COLOURS.get)

# Read the SVG content and display it
svg_content = open("first_solution.svg", "r").read()

# Display the SVG content using HTML in Streamlit
st.write(f"Solution for Date: {number}")
st.components.v1.html(svg_content, width=10000, height=10000)
